...
class Suggestion:
    ...
    def load_data(self):
        """Load all necessary data files and normalize embeddings"""
        print("Loading data...")
        ...
        self.rating_df = create_rating_df(train_arr, test_arr)
    ...
    def initialize_model(self):
        """Initialize predictor model if not provided"""
    ...
    def find_similar_animes(self, name, n=10, consider_genres=True, genre_weight=0.2):
        """Find similar animes based on embedding similarity and optional genre overlap"""
        index = getAnimeFrame(name, self.anime_df).anime_id.values[0]
        encoded_index = self.anime2anime_encoder.get(index)
        
        # Calculate cosine similarity (already normalized embeddings)
        ...
        # CRITICAL FIX: Sort in DESCENDING order
    ...
    def _calculate_anime_scores(self, candidates, dists, query_index, consider_genres, genre_weight):
        """Calculate final scores for anime candidates with genre consideration"""
        ...
        for candidate_idx in candidates:
            ...
            if consider_genres and query_genres:
                ...
                if candidate_genres and query_genres:
                    ...
                    # Improved: Use Jaccard similarity for better genre matching
                    union = len(query_genres.union(candidate_genres))
                    genre_overlap = matching / union if union > 0 else 0
            
            # Combine cosine similarity with genre overlap
    ...
    def _build_anime_result_df(self, final_scores, n):
        """Build result dataframe from scored anime candidates"""
        similarity_arr = []
        
        for rank, item in enumerate(final_scores[:n * 2], 1):  # Get more candidates for filtering
    ...
    def get_user_preferences(self, user_id):
        """Get top-rated animes for a user"""
    ...
    def execute(self, user_pred_dict,num):
        """Main execution pipeline for generating recommendations"""
        print("\n" + "="*60)
        ...
        # Build temp user profile
        print(f"Building profile for user with {len(user_pred_dict)} ratings...")
        self.temp_user_profile = create_temp_user_profile(user_pred_dict,self.anime_df, )
        
        if self.temp_user_profile.empty:
            print("Error: Could not create valid user profile")
            return "404"
        
        # Build inference model and optimize embedding
        ...
        result = create_result_df(self.hybrid_results, n_recommendations=n, df=self.anime_df)
